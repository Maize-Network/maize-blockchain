...
# Gets the version from the result of "maize version"
...
def get_maize_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["maize", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_maize_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/maize-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/maize-blockchain-gui/packages/gui/package.json")
