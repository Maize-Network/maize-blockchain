...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="maize-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="maize-blockchain-linux-arm64"
fi
...
# Builds CLI only rpm
CLI_RPM_BASE="maize-blockchain-cli-$MAIZE_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/maize"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/maize/"
ln -s ../../opt/maize/maize "dist/$CLI_RPM_BASE/usr/bin/maize"
...
fpm -s dir -t rpm \
  ...
  --name maize-blockchain-cli \
  --license Apache-2.0 \
  --version "$MAIZE_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Maize is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
electron-packager . maize-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/maize.icns --overwrite --app-bundle-id=net.maize.blockchain \
--appVersion=$MAIZE_INSTALLER_VERSION --executable-name=maize-blockchain \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create maize-blockchain-$MAIZE_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Maize-Network/maize-blockchain/issues/3846
  ...
  --options.version $MAIZE_INSTALLER_VERSION \
