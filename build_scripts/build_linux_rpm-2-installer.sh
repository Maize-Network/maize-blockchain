...
cd ../maize-blockchain-gui/packages/gui || exit 1
...
echo electron-builder build --linux rpm "${OPT_ARCH}" \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="Maize Blockchain" \
  --config.rpm.packageName="maize-blockchain"
...
GUI_RPM_NAME="maize-blockchain-${MAIZE_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${MAIZE_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"
