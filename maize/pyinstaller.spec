...
ROOT = pathlib.Path(importlib.import_module("maize").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `maize` file name (which is the executable)
    and the `maize` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `maize` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("maize/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("maize-blockchain"))[0]
...
# TODO: collapse all these entry points into one `maize_exec` entrypoint that accepts the server as a parameter

entry_points = ["maize.cmds.maize"] + [f"maize.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "maize.util.bip39"])

if THIS_IS_WINDOWS:
    maize_mod = importlib.import_module("maize")
...
datas.append((f"{ROOT}/maize/util/english.txt", "maize/util"))
datas.append((f"{ROOT}/maize/util/initial-config.yaml", "maize/util"))
datas.append((f"{ROOT}/maize/wallet/puzzles/*.hex", "maize/wallet/puzzles"))
datas.append((f"{ROOT}/maize/ssl/*", "maize/ssl"))
...
add_binary("maize", f"{ROOT}/maize/cmds/maize.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/maize/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/maize/server/start_{server}.py", COLLECT_ARGS)
