...
from maize.wallet.nft_wallet.nft_info import NFTInfo, NFTCoinInfo
...
from maize.wallet.notification_store import Notification
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xmz_target_address")
        pool_target = config["pool"].get("xmz_target_address")
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xmz")
        ...
        key = None
        if asset_id is not None and asset_id != "xmz":
