...
from maize.cmds.passphrase_funcs import obtain_current_passphrase
...
from maize.util.file_keyring import MAX_LABEL_LENGTH
from maize.util.ints import uint32
from maize.util.keychain import Keychain, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'maize keys generate'")
    ...
    if Keychain.needs_migration():
        ...
        if len(keys_to_migrate) > 0:
            ...
            if not prompt_yes_no("Migrate these keys?"):
                ...
                print("Migration aborted, can't run any maize commands.")
                return False
