...
from maize.daemon.client import acquire_connection_to_daemon
from maize.util.config import load_config
from maize.util.errors import KeychainMaxUnlockAttempts
from maize.util.keychain import Keychain, supports_os_passphrase_storage
from maize.util.keyring_wrapper import KeyringWrapper, DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Maize processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("MAIZE_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'maize passphrase set' or 'maize passphrase remove' to update or remove your passphrase")
        sys.exit(1)
