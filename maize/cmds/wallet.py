...
from maize.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", short_help="Send maize to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much maize to send, in XMZ", type=str, required=True)
...
@click.option("-t", "--address", help="Address to send the XMZ", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min_coin_amount",
    help="Ignore coins worth less then this much XMZ or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XMZ/CATs for XMZ/CATs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
