...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        all_targets = []
        stop_searching_for_farmer = "xmz_target_address" not in config["farmer"]
        stop_searching_for_pool = "xmz_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xmz_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xmz_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xmz_target_address") or all_targets[-2] == config["pool"].get(
                    "xmz_target_address"
                ):
        ...
        if "xmz_target_address" not in config["farmer"]:
            print(
                f"Setting the xmz destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["xmz_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["xmz_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['xmz_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "xmz_target_address" not in config["pool"]:
            print(f"Setting the xmz destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["xmz_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["xmz_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['xmz_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the XMZ destination addresses, edit the `xmz_target_address` entries in"
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(maize_ca_crt_path, maize_ca_crt, maize_ca_key_path, maize_ca_key, overwrite=overwrite)
