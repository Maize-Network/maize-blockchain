...
from maize.ssl.create_ssl import generate_ca_signed_cert, get_maize_ca_crt_key, make_ca_cert
...
from maize.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def create_all_ssl(root: Path):
    ...
    maize_ca_crt_path.write_bytes(maize_ca_crt)
    maize_ca_key_path.write_bytes(maize_ca_key)
    ...
    maize_ca_crt, maize_ca_key = get_maize_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, maize_ca_crt, maize_ca_key, False, overwrite=False)
...
def maize_minor_release_number():
    res = int(maize_version_number()[2])
...
def maize_init(root_path: Path):
    if os.environ.get("MAIZE_ROOT", None) is not None:
        print(
            f"warning, your MAIZE_ROOT is set to {os.environ['MAIZE_ROOT']}. "
            f"Please unset the environment variable and run maize init again\n"
