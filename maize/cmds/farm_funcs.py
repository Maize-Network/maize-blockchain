...
from maize.cmds.cmds_util import get_any_service_client
...
from maize.util.misc import format_bytes
from maize.util.misc import format_minutes
from maize.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total maize farmed: {amounts['farmed_amount'] / units['maize']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['maize']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['maize']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'maize start wallet' and 'maize wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'maize wallet show'")
    else:
        print("Note: log into your key using 'maize wallet show' to see rewards for each key")
