...
from maize.plotting.util import add_plot_directory, validate_plot_size
...
def show_plots(root_path: Path):
    from maize.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'maize plots add -d [dir]' and remove with"
        + " 'maize plots remove -d [dir]'"
        + " Scan and check plots with 'maize plots check'"
    )
    print()
...
def create_cmd(
    ...
    from maize.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from maize.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from maize.plotting.util import remove_plot_directory
