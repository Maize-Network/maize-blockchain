...
from maize.cmds.init_funcs import create_all_ssl, create_default_maize_config
...
from maize.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
from maize.consensus.condition_costs import ConditionCost
from maize.consensus.constants import ConsensusConstants
from maize.consensus.default_constants import DEFAULT_CONSTANTS
from maize.consensus.deficit import calculate_deficit
...
from maize.consensus.pot_iterations import (
    calculate_ip_iters,
...
from maize.consensus.vdf_info_computation import get_signage_point_vdf_info
from maize.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from maize.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from maize.full_node.generator import setup_generator_args
from maize.full_node.mempool_check_conditions import GENERATOR_MOD
from maize.full_node.signage_point import SignagePoint
from maize.plotting.create_plots import PlotKeys, create_plots
from maize.plotting.manager import PlotManager
from maize.plotting.util import (
    PlotRefreshEvents,
    PlotRefreshResult,
...
from maize.simulator.wallet_tools import WalletTool
...
from maize.util.config import config_path_for_filename, load_config, lock_config, override_config, save_config
...
from maize.util.keychain import Keychain, bytes_to_mnemonic
from maize.util.prev_transaction_block import get_prev_transaction_block
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, self.log, user="testing-1.8.0", service="maize-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `maize keys generate`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "xmz")
