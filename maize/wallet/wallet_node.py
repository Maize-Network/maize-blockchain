...
from maize.util.errors import Err, ValidationError
from maize.util.ints import uint32, uint128
from maize.util.keychain import Keychain
from maize.util.lru_cache import LRUCache
from maize.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from maize.util.path import mkdir, path_from_root
from maize.wallet.block_record import HeaderBlockRecord
from maize.wallet.derivation_record import DerivationRecord
from maize.wallet.settings.settings_objects import BackupInitialized
from maize.wallet.transaction_record import TransactionRecord
from maize.wallet.util.backup_utils import open_backup_file
from maize.wallet.util.wallet_types import WalletType
from maize.wallet.wallet_action import WalletAction
from maize.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSMaizeConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSMaizeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSMaizeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSMaizeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSMaizeConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
