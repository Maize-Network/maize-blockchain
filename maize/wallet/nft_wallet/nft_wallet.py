...
from maize.wallet.nft_wallet.nft_off_chain import delete_off_chain_metadata, get_off_chain_metadata
from maize.wallet.nft_wallet.nft_puzzles import (
...
from maize.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'XMZ'} to offer")
