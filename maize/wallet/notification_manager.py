...
from maize.wallet.notification_store import Notification, NotificationStore
from maize.wallet.transaction_record import TransactionRecord
from maize.wallet.util.compute_memos import compute_memos_for_spend
from maize.wallet.util.notifications import construct_notification
...
class NotificationManager:
    ...
    async def send_new_notification(
        ...
        maize_tx = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
            amount,
            notification_hash,
            fee,
            coins=coins,
        ...
        full_tx: TransactionRecord = dataclasses.replace(
            maize_tx, spend_bundle=SpendBundle.aggregate([maize_tx.spend_bundle, extra_spend_bundle])
        )
        return full_tx
