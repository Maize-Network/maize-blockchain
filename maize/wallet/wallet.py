...
from maize.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from maize.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_maize(self, maize_amount: int, exclude: List[Coin]) -> SpendBundle:
        ...
        if maize_amount < 0:
            utxos = await self.select_coins(abs(maize_amount), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": maize_amount}]
