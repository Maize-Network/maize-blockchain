...
from maize.protocols.protocol_state_machine import message_requires_reply
from maize.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from maize.protocols.shared_protocol import protocol_version
from maize.server.introducer_peers import IntroducerPeers
from maize.server.outbound_message import Message, NodeType
from maize.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from maize.util.network import is_in_network, is_localhost, select_port
from maize.util.ssl_check import verify_ssl_certs_and_keys
...
class MaizeServer:
    def __init__(
        ...
        maize_ca_crt_key: Tuple[Path, Path],
        name: str = None,
    ):
        # Keeps track of all connections to and from this node.
        self.all_connections: Dict[bytes32, WSMaizeConnection] = {}
        ...
        self.maize_ca_crt_path, self.maize_ca_key_path = maize_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSMaizeConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.maize_ca_crt_path, self.maize_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSMaizeConnection] = None
        try:
            connection = WSMaizeConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSMaizeConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.maize_ca_crt_path, self.maize_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSMaizeConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSMaizeConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSMaizeConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSMaizeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSMaizeConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSMaizeConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSMaizeConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSMaizeConnection, trusted_peers: Dict) -> bool:
