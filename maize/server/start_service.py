...
from maize.daemon.server import service_launch_lock_path
from maize.util.lock import Lockfile, LockfileError
from maize.server.ssl_context import maize_ssl_ca_paths, private_ssl_ca_paths
...
from maize.rpc.rpc_server import start_rpc_server, RpcServer
...
class Service:
    def __init__(
        ...
        *,
        ...
        self._log.info(f"maize-blockchain version: {maize_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        maize_ca_crt, maize_ca_key = maize_ssl_ca_paths(root_path, self.config)
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"maize_{self._service_name}"
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for MaizeServer to be closed")
