...
from maize.protocols import full_node_protocol, introducer_protocol
from maize.protocols.protocol_message_types import ProtocolMessageTypes
from maize.server.address_manager import AddressManager, ExtendedPeerInfo
from maize.server.address_manager_store import AddressManagerStore
from maize.server.address_manager_sqlite_store import create_address_manager_from_db
from maize.server.outbound_message import NodeType, make_msg, Message
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: MaizeServer = server
    ...
    async def on_connect(self, peer: ws.WSMaizeConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSMaizeConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSMaizeConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSMaizeConnection):
