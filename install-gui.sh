...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the maize python virtual environment."
  echo "Execute '. ./activate' before running."
  exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Maize Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in maize-blockchain-gui
SUBMODULE_BRANCH=$1
...
# https://github.com/Maize-Network/maize-blockchain/pull/10460#issuecomment-1054492495
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `maize version` to version property of GUI's package.json
...
echo ""
echo "Maize blockchain install-gui.sh completed."
echo ""
