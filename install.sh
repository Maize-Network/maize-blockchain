...
if [ "$(uname -m)" = "armv7l" ]; then
  ...
  echo "https://github.com/Maize-Network/maize-blockchain/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
# Get submodules
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with maize
  ...
  # yum install python3 brings Python3.6 which is not supported by maize
  cd ..
      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.11.* ]]; then
            echo "Maize requires Python version < 3.11.0" >&2
...
echo "https://github.com/Maize-Network/maize-blockchain/wiki/Quick-Start-Guide"
echo ""
echo "To install the GUI type 'sh install-gui.sh' after '. ./activate'."
echo ""
echo "Type '. ./activate' and then 'maize init' to begin."
