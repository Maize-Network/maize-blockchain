...
import { type OfferSummaryRecord } from '@maize/api';
import {
  Flex,
  FormatLargeNumber,
  StateColor,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xmz', 'txmz'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToMaize(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xmz', 'txmz'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToMaize(takerEntries[0][1])
