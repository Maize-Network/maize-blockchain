...
import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@maize/api-react'
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = maizeToMojo(fee);
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={mojoToMaizeLocaleString(balance)}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {address}
