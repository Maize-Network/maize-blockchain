...
const getMaizeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('maize');
  // first see if we can get a maize exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or maize is on the path
    try {
      version = child_process
