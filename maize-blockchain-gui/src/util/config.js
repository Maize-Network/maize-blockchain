...
global.daemon_rpc_ws = `wss://${self_hostname}:55410`;
...
function loadConfig(net) {
  try {
    // check if MAIZE_ROOT is set. it overrides 'net'
    const config_root_dir =
      'MAIZE_ROOT' in process.env
        ? process.env.MAIZE_ROOT
        : path.join(os.homedir(), '.maize', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55410); // jshint ignore:line
