...
export default function Block() {
  ...
  const poolReward = mojo_to_maize(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_maize(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const maizeFees = blockRecord.fees
    ? mojo_to_maize(BigInt(blockRecord.fees))
    : '';
