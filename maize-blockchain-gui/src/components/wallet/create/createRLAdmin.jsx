...
import { AlertDialog } from '@maize/core';
import {
  Typography,
  Button,
  Box,
...
import { maize_to_mojo } from '../../../util/maize';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      maizeper_input.value === '' ||
      Number(maizeper_input.value) === 0 ||
      !Number(maizeper_input.value) ||
      isNaN(Number(maizeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const maizeper = maize_to_mojo(maizeper_input.value);
    const maizeper_value = Number.parseInt(Number(maizeper));
    const userpubkey = userpubkey_input.value;
    const amount = maize_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = maize_to_mojo(fee_input.value);
