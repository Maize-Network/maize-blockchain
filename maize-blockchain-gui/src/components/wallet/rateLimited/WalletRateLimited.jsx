...
import { AlertDialog, Card, Flex } from '@maize/core';
import {
  send_transaction,
  rl_set_user_info_action,
} from '../../../modules/message';
import { mojo_to_maize_string, maize_to_mojo } from '../../../util/maize';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const maizeper_input = ip_parsed.limit;
    ...
    const maizeper_value = Number.parseInt(Number(maizeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (maize per interval):{' '}
                  {mojo_to_maize_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_maize_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = maize_to_mojo(fee_input.value);
