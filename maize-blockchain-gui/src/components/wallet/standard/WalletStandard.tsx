  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of maize in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.slice(0, 12) === 'maize_addr://') {
    ...
    const amountValue = Number.parseFloat(maize_to_mojo(amount));
    const feeValue = Number.parseFloat(maize_to_mojo(fee));
