...
from maize.protocols.shared_protocol import capabilities, protocol_version
...
async def disconnect_all(server: MaizeServer) -> None:
    cons = list(server.all_connections.values())[:]
    for con in cons:
        await con.close()


async def disconnect_all_and_reconnect(server: MaizeServer, reconnect_to: MaizeServer, self_hostname: str) -> bool:
...
async def add_dummy_connection(
    server: MaizeServer, self_hostname: str, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.maize_ca_crt_path.read_bytes(), server.maize_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.maize_ca_crt_path, server.maize_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: MaizeServer, server_2: MaizeServer, self_hostname: str) -> WSMaizeConnection:
    """
