...
from maize.protocols import farmer_protocol
from maize.protocols.harvester_protocol import Plot
from maize.rpc.farmer_rpc_api import (
    FilterItem,
...
from maize.rpc.harvester_rpc_client import HarvesterRpcClient
from maize.simulator.block_tools import get_plot_dir
from maize.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from maize.util.config import load_config, lock_and_load_config, save_config
from maize.util.hash import std_hash
from maize.util.ints import uint8, uint32, uint64
from maize.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "xmz"), encode_puzzle_hash(new_ph_2, "xmz"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "xmz"), encode_puzzle_hash(observer_pool, "xmz")
    )
    ...
    assert config["farmer"]["xmz_target_address"] == encode_puzzle_hash(observer_farmer, "xmz")
    assert config["pool"]["xmz_target_address"] == encode_puzzle_hash(observer_pool, "xmz")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "xmz")
