...
from tests.core.data_layer.util import MaizeRoot

pytestmark = pytest.mark.data_layer


@pytest.mark.asyncio
async def test_help(maize_root: MaizeRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = maize_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(maize_root: MaizeRoot, maize_daemon: None, maize_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with maize_root.print_log_after():
        create = maize_root.run(args=["data", "create_data_store"])
