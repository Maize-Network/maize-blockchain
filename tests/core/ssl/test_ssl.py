...
from maize.protocols.shared_protocol import protocol_version, capabilities
from maize.server.outbound_message import NodeType
from maize.server.server import MaizeServer, ssl_context_for_client
from maize.server.ws_connection import WSMaizeConnection
from maize.ssl.create_ssl import generate_ca_signed_cert
from maize.types.peer_info import PeerInfo
from maize.util.ints import uint16


async def establish_connection(server: MaizeServer, self_hostname: str, ssl_context) -> None:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node_sim_and_wallet, self_hostname):
        ...
        server_1: MaizeServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, farmer_one_harvester, self_hostname):
        ...
        generate_ca_signed_cert(
            farmer_server.maize_ca_crt_path.read_bytes(), farmer_server.maize_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.maize_ca_crt_path, farmer_server.maize_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_full_node(self, wallet_node_sim_and_wallet, self_hostname):
        ...
        generate_ca_signed_cert(
            full_node_server.maize_ca_crt_path.read_bytes(),
            full_node_server.maize_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.maize_ca_crt_path, full_node_server.maize_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node_sim_and_wallet, self_hostname):
        ...
        generate_ca_signed_cert(
            wallet_server.maize_ca_crt_path.read_bytes(), wallet_server.maize_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.maize_ca_crt_path, wallet_server.maize_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, farmer_one_harvester, self_hostname):
        ...
        generate_ca_signed_cert(
            harvester_server.maize_ca_crt_path.read_bytes(),
            harvester_server.maize_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.maize_ca_crt_path, harvester_server.maize_ca_key_path, pub_crt, pub_key
        )
        with pytest.raises(aiohttp.ClientConnectorError):
    ...
    async def test_introducer(self, introducer, self_hostname):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.maize_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.maize_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.maize_ca_crt_path.read_bytes(),
            introducer_server.maize_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.maize_ca_crt_path, introducer_server.maize_ca_key_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord, self_hostname):
        ...
        generate_ca_signed_cert(
            timelord_server.maize_ca_crt_path.read_bytes(),
            timelord_server.maize_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.maize_ca_crt_path, timelord_server.maize_ca_key_path, pub_crt, pub_key
        )
        with pytest.raises(aiohttp.ClientConnectorError):
