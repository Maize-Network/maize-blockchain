...
from maize.cmds.wallet_funcs import print_offer_summary
...
@pytest.mark.asyncio
async def test_print_offer_summary_xmz(capsys: Any) -> None:
    summary_dict = {"xmz": 1_000_000_000_000}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "XMZ (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xmz_and_cats(capsys: Any) -> None:
    summary_dict = {
        "xmz": 2_500_000_000_000,
    ...
    assert "XMZ (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xmz_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "xmz": 0,
    ...
    assert "XMZ (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.asyncio
async def test_print_offer_summary_xmz_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"xmz": 1}
    ...
    assert "XMZ (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
