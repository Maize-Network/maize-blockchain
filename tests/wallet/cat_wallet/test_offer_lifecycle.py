...
from maize.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XMZ coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            maize_coins: List[Coin] = all_coins[None]
            ...
            # Create an XMZ Offer for RED
            maize_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            maize_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                maize_requested_payments, maize_coins
            )
            maize_announcements: List[Announcement] = Offer.calculate_announcements(maize_requested_payments, driver_dict)
            maize_secured_bundle: SpendBundle = generate_secure_bundle(maize_coins, maize_announcements, 1000)
            maize_offer = Offer(maize_requested_payments, maize_secured_bundle, driver_dict)
            assert not maize_offer.is_valid()

            # Create a RED Offer for XMZ
            ...
            new_offer = Offer.aggregate([maize_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for XMZ and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xmz": 900, str_to_tail_hash("red").hex(): 350},
