...
async def test_default_transfer_program(setup_sim: Tuple[SpendSim, SimClient]) -> None:
    ...
    try:
        ...
        xmz_coin = (await sim_client.get_coin_records_by_puzzle_hash(ACS_PH, include_spent_coins=False))[0].coin
        ...
        xmz_announcement_spend = CoinSpend(
            xmz_coin,
            ACS,
        ...
        for i in range(1, 3):
            for announcement_combo in itertools.combinations(
                [did_announcement_spend, xmz_announcement_spend, cat_announcement_spend], i
            ):
        ...
        full_bundle = SpendBundle(
            [ownership_spend, did_announcement_spend, xmz_announcement_spend, cat_announcement_spend], G2Element()
        )
