...
async def test_nft_offer_with_fee(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 xmz
    ...
    xmz_request = 100
    maker_fee = uint64(10)
    offer_nft_for_xmz = {wallet_maker.id(): xmz_request, nft_asset_id: -1}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_xmz, driver_dict, fee=maker_fee
    )
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + xmz_request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - xmz_request - taker_fee)
    ...
    # MAKE SECOND TRADE: 100 xmz for 1 NFT
    ...
    xmz_offered = 1000
    maker_fee = uint64(10)
    offer_xmz_for_nft = {wallet_maker.id(): -xmz_offered, nft_to_buy_asset_id: 1}
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_xmz_for_nft, driver_dict_to_buy, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
    ...
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - xmz_offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + xmz_offered - taker_fee)
...
async def test_nft_offer_nft_for_nft(two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.maize.net/image2.html"]),
