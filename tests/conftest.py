...
from maize.clvm.spend_sim import SimClient, SpendSim
...
from maize.util.config import create_default_maize_config, lock_and_load_config
...
from maize.simulator.block_tools import BlockTools, test_constants, create_block_tools, create_block_tools_async
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the maize root directory
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, xmz_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, xmz_spam_amount):
        yield _
...
@pytest.fixture(scope="function")
def tmp_maize_root(tmp_path):
    """
    Create a temp directory and populate it with an empty maize_root directory.
    """
    path: Path = tmp_path / "maize_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_maize_root) -> Path:
    """
    Create a temp maize_root directory and populate it with a default config.yaml.
    Returns the maize_root path.
    """
    root_path: Path = tmp_maize_root
